...
- хранить состояние задач в базе данных или Redis;
- добавить интеграционные тесты общения manager/worker;
- улучшить стратегию разбиения пространства перебора;
- добавить retry/circuit breaker для сетевых вызовов;
- вынести больше параметров в конфигурацию.

## Что попадает в репозиторий

В репозиторий должны входить только исходники и конфигурация проекта:
...
Сборочные артефакты, локальные кэши, `build/`, `.gradle/`, `bin/`, отчеты и IDE-файлы игнорируются через `.gitignore`.
